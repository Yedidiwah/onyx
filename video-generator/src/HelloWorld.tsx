import { AbsoluteFill, useCurrentFrame, useVideoConfig, spring, Video, staticFile, Series, Img, interpolate, Audio } from "remotion";
...
export const HelloWorld: React.FC<{
    ...
    // Fade out לטקסט החל מפריים 435 לקראת הלוגו
    const textOpacityOut = interpolate(frame, [435, 450], [1, 0], { extrapolateRight: "clamp" });

    // אנימציות כניסה לטקסט (בכפל עם שקיפות היציאה)
    const finalTitleOpacity = spring({ frame: frame - 15, fps, config: { damping: 100 } }) * textOpacityOut;
    const titleY = spring({ frame: frame - 15, fps, from: 50, to: 0 });
    const finalSubOpacity = spring({ frame: frame - 45, fps, config: { damping: 100 } }) * textOpacityOut;
    const subScale = spring({ frame: frame - 45, fps, from: 0.8, to: 1 });
    const finalLinkOpacity = spring({ frame: frame - 90, fps }) * textOpacityOut;
    ...
    return (
        <AbsoluteFill className="bg-black text-white font-sans">
            ...
            {/* מוזיקת הרקע! startFrom חותך את ה-10 שניות הראשונות (300 פריימים) כדי להגיע ישר לקצב */}
            <Audio src={staticFile(music)} volume={0.4} startFrom={300} />
            ...
            <AbsoluteFill className="flex flex-col items-center justify-center p-10 pointer-events-none">
                <div style={{ opacity: finalTitleOpacity, transform: `translateY(${titleY}px)` }} className="flex flex-col items-center mb-16">
                ...
                <div style={{ opacity: finalSubOpacity, transform: `scale(${subScale})` }} className="bg-black/50 backdrop-blur-md border border-white/20 px-16 py-8 rounded-[3rem] shadow-2xl mt-4">
                    <p className="text-8xl font-medium text-emerald-400 drop-shadow-md">
                        {subTitleToReplace}
                    </p>
                </div>

                <div style={{ opacity: finalLinkOpacity }} className="absolute bottom-24 text-5xl font-light tracking-widest text-white/90 uppercase drop-shadow-md">
                    Link in bio to book
                </div>
            </AbsoluteFill>
        </AbsoluteFill>
